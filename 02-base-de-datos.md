...
SQL Server** (`AgendaDigital`). Este bloque documenta el modelo Entidad-Relación y el
diccionario de datos de cada tabla.

> Además de las 20 tablas de dominio, la BD física contiene 12 tablas de
> infraestructura que no forman parte del modelo de la aplicación: las `auth_*` y
> `django_*` que crea Django (admin, sesiones, permisos, migraciones), la
> `authtoken_token` de DRF y `sysdiagrams` (diagramas de SSMS).
...
- **Plantel usa UUID.** La PK de `Plantel` es de tipo `uniqueidentifier` (UUID). Por
  ...
  entera.
- **Reglas `ON DELETE`.** La integridad referencial en el borrado (CASCADE,
  PROTECT, SET NULL) se aplica en la capa de aplicación (ORM de Django), que es la
  vía única de acceso a los datos. Las FK hacia `Plantel` materializan además su
  regla directamente en el motor (`ON DELETE CASCADE`; `SET NULL` en
  `SolicitudAdmin`).
- **Valores por defecto.** Los valores por defecto (`activo = 1`,
  `audiencia = 'todos'`, …) y las fechas automáticas (`auto_now_add` / `auto_now`)
  se asignan en la capa de aplicación al crear o guardar cada registro.
...
| `DateField` | `date` |
| `TimeField` | `time(7)` |
| `DateTimeField` | `datetimeoffset(7)` |
| `JSONField` | `nvarchar(max)` + `CHECK (ISJSON(…) = 1)` |
...
> Un ✔ indica que la propiedad aplica; una celda vacía indica que no.
...
erDiagram
    ...
    Letra {
        nvarchar id_letra PK
    }
    Grupo {
        bigint id_grupo PK
        int semestre_id FK
        nvarchar letra_id FK
    }
    Usuario {
...
- Restricción de unicidad: **(`semestre_id`, `letra_id`)** — no puede existir dos
  veces el mismo grupo escolar.
- Ambas FK con `ON DELETE CASCADE`: al eliminar un semestre o una letra se eliminan
  sus grupos.
...
| password_mock | nvarchar | 128 | | | | ✔ | |
| id_api | nvarchar | 100 | | | | ✔ | |
| activo | bit | — | | | ✔ | | |
| ultima_sesion | datetimeoffset(7) | — | | | | ✔ | |

- Son **únicos**: `correo` (identificador de acceso del personal), `google_id`
  (vinculación con la cuenta de Google) y `matricula` (identificador del alumno).
- `rol_id` con `ON DELETE PROTECT`: no puede eliminarse un rol que tenga usuarios
  asociados.
...
| Atributo | Tipo | Tamaño | PK | FK | Oblig. | Null | Autoinc. |
|---|---|---|:--:|:--:|:--:|:--:|:--:|
| id_usuario_plantel | bigint | — | ✔ | | ✔ | | ✔ |
| usuario_id | bigint | — | | ✔ | ✔ | | |
| plantel_id | uniqueidentifier | — | | ✔ | ✔ | | |
| turno_id | bigint | — | | ✔ | ✔ | | |

- Restricción de unicidad: **(`usuario_id`, `plantel_id`, `turno_id`)** — un usuario
  no puede tener dos veces la misma adscripción.
- Las tres FK con `ON DELETE CASCADE`: al eliminar el usuario, el plantel o el
  turno se eliminan sus adscripciones.
...
| id | bigint | — | ✔ | | ✔ | | ✔ |
| usuario_id | bigint | — | | ✔ | ✔ | | |
| email_google | nvarchar | 254 | | | | ✔ | |
| access_token | nvarchar(max) | máx | | | ✔ | | |
| refresh_token | nvarchar(max) | máx | | | | ✔ | |
| scopes | nvarchar(max) | máx | | | ✔ | | |
| expiry | datetimeoffset(7) | — | | | ✔ | | |
| semestre_id | int | — | | ✔ | | ✔ | |
| grupo_id | bigint | — | | ✔ | | ✔ | |

- `usuario_id` es **único** (relación 1:1) con `ON DELETE CASCADE`: cada usuario
  tiene a lo sumo una credencial y esta se elimina junto con él.
- `semestre_id` y `grupo_id` con `ON DELETE SET NULL`; solo se utilizan en alumnos,
  para filtrar qué eventos se sincronizan a su Google Calendar.
...
| temas | nvarchar(max) | máx | | | ✔ | | |
| activo | bit | — | | | ✔ | | |
| fecha_registro | datetimeoffset(7) | — | | | ✔ | | |
| fecha_actualizacion | datetimeoffset(7) | — | | | ✔ | | |

- `token_fcm` es **único**: identifica al dispositivo ante Firebase Cloud Messaging.
- `temas` es un arreglo JSON (lista de temas suscritos); por defecto `[]`. La BD
  valida su formato con `CHECK (ISJSON(temas) = 1)`.
...
| publico | bit | — | | | | ✔ | |
| fecha_creacion | datetimeoffset(7) | — | | | ✔ | | |

- `calendario_id` con `ON DELETE CASCADE`; `tipo_evento_id` con `ON DELETE PROTECT`
  (no puede eliminarse un tipo con eventos asociados).
- `plantel_id` y `turno_id` con `ON DELETE CASCADE`; `semestre_id`, `grupo_id` y
  `creado_por_id` con `ON DELETE SET NULL` (el evento sobrevive y conserva su
  información aunque desaparezca el dato relacionado).
...
- Restricción de unicidad: **(`evento_id`, `usuario_id`)** — cada evento se registra
  una sola vez por usuario sincronizado.
...
| id_anuncio | bigint | — | ✔ | | ✔ | | ✔ |
| titulo | nvarchar | 200 | | | ✔ | | |
| descripcion | nvarchar(max) | máx | | | ✔ | | |
| color | nvarchar | 20 | | | ✔ | | |
| audiencia | nvarchar | 20 | | | ✔ | | |
| plantel_id | uniqueidentifier | — | | ✔ | | ✔ | |
| turno_id | bigint | — | | ✔ | | ✔ | |
| creado_por_id | bigint | — | | ✔ | | ✔ | |
| fecha_creacion | datetimeoffset(7) | — | | | ✔ | | |
...
- `plantel_id` y `turno_id` con `ON DELETE CASCADE`; `creado_por_id` con
  `ON DELETE SET NULL` (el anuncio se conserva aunque se elimine a su autor).
...
| participante_a_id | bigint | — | | ✔ | ✔ | | |
| participante_b_id | bigint | — | | ✔ | ✔ | | |
| plantel_id | uniqueidentifier | — | | ✔ | ✔ | | |
| fecha_creacion | datetimeoffset(7) | — | | | ✔ | | |
| activa | bit | — | | | ✔ | | |

- Restricción de unicidad: **(`participante_a_id`, `participante_b_id`)** — solo
  puede existir una conversación por par de usuarios.
...
| remitente_id | bigint | — | | ✔ | ✔ | | |
| contenido_cifrado | nvarchar(max) | máx | | | ✔ | | |
| iv | nvarchar | 24 | | | ✔ | | |
| metadatos_cifrados | nvarchar(max) | máx | | | | ✔ | |
| iv_metadatos | nvarchar | 24 | | | | ✔ | |
| fecha_envio | datetimeoffset(7) | — | | | ✔ | | |
| eliminado | bit | — | | | ✔ | | |
...
- Ambas FK con `ON DELETE CASCADE`. `eliminado` por defecto `0` (borrado lógico).
...
| Atributo | Tipo | Tamaño | PK | FK | Oblig. | Null | Autoinc. |
|---|---|---|:--:|:--:|:--:|:--:|:--:|
| id_lectura_mensaje | bigint | — | ✔ | | ✔ | | ✔ |
| conversacion_id | bigint | — | | ✔ | ✔ | | |
| usuario_id | bigint | — | | ✔ | ✔ | | |
| ultimo_leido_id | bigint | — | | ✔ | | ✔ | |

- Restricción de unicidad: **(`conversacion_id`, `usuario_id`)** — una sola marca de
  lectura por usuario en cada conversación.
...
| Atributo | Tipo | Tamaño | PK | FK | Oblig. | Null | Autoinc. |
|---|---|---|:--:|:--:|:--:|:--:|:--:|
| id_solicitud_admin | bigint | — | ✔ | | ✔ | | ✔ |
| tipo | nvarchar | 20 | | | ✔ | | |
| usuario_id | bigint | — | | ✔ | ✔ | | |
| plantel_id | uniqueidentifier | — | | ✔ | | ✔ | |
| turno_id | bigint | — | | ✔ | | ✔ | |
| motivo | nvarchar(max) | máx | | | ✔ | | |
| estado | nvarchar | 20 | | | ✔ | | |
| fecha_solicitud | datetimeoffset(7) | — | | | ✔ | | |
| resuelta_por_id | bigint | — | | ✔ | | ✔ | |
| fecha_resolucion | datetimeoffset(7) | — | | | | ✔ | |
...
- `usuario_id` con `ON DELETE CASCADE`; `plantel_id`, `turno_id` y `resuelta_por_id`
  con `ON DELETE SET NULL` (la solicitud conserva su historial aunque desaparezca el
  plantel, el turno o el resolutor).
